"""Run the SG-IA ground-truth benchmark against the RAG HTTP API.

The chatbot and the LLM judge are deliberately separate concerns: questions go
to the running RAG API, while a separately configured Bedrock model evaluates
...
class RagApiResponseError(EvaluationError):
    """Raised for a non-success response from the RAG API."""
...
class RagApiClient:
    ...
    def _request(self, method: str, path: str, **kwargs: Any) -> Any:
        try:
            ...
            raise EvaluationError(f"Cannot reach the RAG API at {self.base_url}: {exc}") from exc
        if not response.ok:
            try:
            ...
            raise RagApiResponseError(
                f"RAG API {path} failed ({response.status_code}): {detail}",
                status_code=response.status_code,
            )
        try:
            return response.json()
        except ValueError as exc:
            raise EvaluationError(f"RAG API {path} returned invalid JSON.") from exc

    def health(self) -> dict[str, Any]:
        value = self._request("GET", "/health", timeout=(3.0, 30.0))
        if not isinstance(value, dict):
            raise EvaluationError("RAG /health returned an invalid response.")
        return value

    def documents(self) -> list[dict[str, Any]]:
        value = self._request("GET", "/documents", timeout=(3.0, 60.0))
        if not isinstance(value, list) or not all(isinstance(item, dict) for item in value):
            raise EvaluationError("RAG /documents returned an invalid response.")
        return value

    def models(self) -> dict[str, Any]:
        value = self._request("GET", "/models", timeout=(3.0, 30.0))
        if not isinstance(value, dict):
            raise EvaluationError("RAG /models returned an invalid response.")
        return value
    ...
    def chat(self, question: str) -> tuple[dict[str, Any], float]:
        ...
        if not isinstance(value, dict) or not isinstance(value.get("answer"), str):
            raise EvaluationError("RAG /chat returned an invalid response.")
        raw_status = str(value.get("status", "answered"))
        value["raw_status"] = raw_status
        if raw_status == "insufficient_evidence":
            value["status"] = "insufficient_knowledge"
...
def normalize_source_path(value: Any) -> str:
    # The shared fixture records corpus-relative paths, whereas the RAG API
    # intentionally exposes only the source filename. Compare their basenames
    # and ignore punctuation introduced by safe upload-name normalization.
    path = str(value or "").strip().replace("\\", "/").rsplit("/", 1)[-1]
    return "".join(character for character in path.casefold() if character.isalnum())
...
def citation_metrics(case: Mapping[str, Any], response: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(citations, list):
        for citation in citations:
            ...
            if source_path:
                actual.add(normalize_source_path(source_path))
...
def collect_rag_evidence(
    ...
    *,
    ...
    judge_chunks: list[dict[str, Any]] = []
    metadata: list[dict[str, Any]] = []
    citations = response.get("citations", [])
    cited_by_chunk: dict[str, Mapping[str, Any]] = {}
    if isinstance(citations, list):
        cited_by_chunk = {
    ...
    retrieved = nested(response, "debug", "retrieved_chunks", default=[])
    if not isinstance(retrieved, list):
        return judge_chunks, metadata
    ...
    for item in retrieved:
        ...
        citation = cited_by_chunk.get(chunk_id)
        if citation is None:
            continue
        content = str(item.get("text", ""))
        limit = min(max_chars_per_chunk, remaining)
        included = content[:limit]
        truncated = len(included) < len(content)
        digest = hashlib.sha256(content.encode("utf-8")).hexdigest()
        source_path = str(item.get("filename") or citation.get("source_path") or "")
        evidence_id = str(citation.get("evidence_id") or chunk_id)
        judge_chunks.append(
            {
...
def classify_result(
    ...
    if score <= 3:
        if recall is None or float(recall) == 0:
            flags.append("RAG_LOOKUP_FAILURE")
        else:
...
def judge_payload(
    ...
    return {
        ...
        "consulted_evidence_chunks": list(evidence_pages),
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--api-url", help="RAG API base URL.")
    ...
    parser.add_argument("--skip-preflight", action="store_true", help="Skip RAG health, model, and document validation.")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    api_url = args.api_url or os.getenv("RAG_API_URL") or config.get("api_url") or "http://127.0.0.1:8001"
    ...
    configured_credentials = nested(config, "judge", "aws_credentials_file")
    credentials_file = (
        args.aws_credentials_file.resolve()
        if args.aws_credentials_file
        else resolve_config_path(configured_credentials, config_dir=config_dir)
        if configured_credentials
        else DEFAULT_AWS_CREDENTIALS
        if DEFAULT_AWS_CREDENTIALS.is_file()
        else None
    )
    judge_model_id = (
        args.judge_model_id
    ...
    if not args.skip_preflight:
        ...
        if health.get("status") != "ok":
            raise EvaluationError("The RAG backend health check is not healthy.")
        if health.get("qdrant") != "reachable":
            raise EvaluationError("The RAG backend cannot reach Qdrant.")
        documents = rag_client.documents()
        if not documents:
            raise EvaluationError("The RAG index contains no documents.")
        models = rag_client.models()
        if str(health.get("generation_model_id", "")) == str(judge_model_id):
            print(
    ...
    max_chars_per_chunk = int(nested(config, "run", "max_evidence_chars_per_chunk", default=12000))
    ...
    for case in cases:
        for repetition in range(1, repetitions + 1):
            ...
            try:
                response, client_latency = rag_client.chat(str(case["question"]))
                record["chatbot"] = response
                record["client_latency_ms"] = client_latency
                record["chatbot_attempts"] = rag_client.last_chat_attempts
            ...
            evidence_pages, evidence_metadata = collect_rag_evidence(
                response,
                max_chars_per_chunk=max_chars_per_chunk,
                max_total_chars=max_total_chars,
            )
            record["rag_evidence"] = evidence_metadata
            try:
    ...
    corpus_documents = documents if documents is not None else []
    corpus_snapshot = [
        {
        ...
        for item in sorted(corpus_documents, key=lambda value: str(value.get("filename", "")))
    ]
    corpus_snapshot_hash = hashlib.sha256(
        json.dumps(corpus_snapshot, ensure_ascii=False, sort_keys=True).encode("utf-8")
    ).hexdigest()
    manifest = {
        "run_id": run_id,
        ...
        "corpus_manifest": {
            "document_count": len(corpus_snapshot),
            "chunk_count": sum(int(item.get("chunk_count") or 0) for item in corpus_snapshot),
            "sha256": corpus_snapshot_hash,
            "documents": corpus_snapshot,
        },
        "judge": {
            "model_id": judge.model_id,
        ...
        "run": {
            ...
            "max_evidence_chars_per_chunk": max_chars_per_chunk,
            "max_total_evidence_chars": max_total_chars,
            "chat_max_attempts": rag_client.chat_max_attempts,
            "chat_retry_delay_seconds": rag_client.chat_retry_delay_seconds,
